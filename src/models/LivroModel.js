...
function validarAnoPublicacao(ano) {
    ...
    if (isNaN(anoNum) || anoNum < 1000 || anoNum > anoAtual) {
        throw new Error(
            `O ano de publicação deve ser entre 1000 e ${anoAtual}.`
        );
    }
...
export default class LivroModel {
    constructor({
        ...
        this.caracteristicas_literarias_pt =
            caracteristicas_literarias_pt;

        this.caracteristicas_literarias_en =
    ...
    static async buscarTodos(filtros = {}) {
        ...
        if (filtros.anoPublicacao) {
            where.anoPublicacao = parseInt(
                filtros.anoPublicacao,
                10
            );
        }

        if (filtros.usuarioId) {
            where.usuarioId = parseInt(
                filtros.usuarioId,
                10
            );
        }
